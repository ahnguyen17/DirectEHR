...
export default function EditOrder() {
    ...
    const fetchOrder = async () => {
      ...
      try {
        ...
        if (!foundOrder) {
          ...
          // If it's a medication order, check if we should show the medication modal
          if (foundOrder.type === 'Medication') {
            // Find the patient data
            const patient = patients.find(p => p.id === foundOrder.patientId);
            if (patient) {
  ...
  const handleSaveMedicationOrder = async (medicationData, existingOrder) => {
    ...
    try {
      ...
      setErrors({
        ...errors,
        submit: 'Failed to update medication order. Please try again.'
      });
    } finally {
      setLoading(false);
    }
